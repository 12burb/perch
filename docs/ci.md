...
| `laptop-smoke` | Linux, macOS, Windows: `bun test apps/cli apps/runner` — `perch dev` on PGlite with the in-process runner, doctor, backup, restore, and the compiled binary serving the embedded web app |
...
3. `publish`: the GitHub release (pre-release when the tag has a suffix such as `v0.2.0-rc.1`) with the binaries, checksums, `openapi.json`, and SBOMs; `npm publish perch-dev` when `NPM_TOKEN` is set (`--tag next` for pre-releases).
